...
class NewBrandWindow:
    def __init__(self, user_id):
        self.window = load_xaml("NewBrandWindow.xaml")
    ...
    def set_events(self):
        ...
        self.txt_title = self.window.FindName("BrandTextBox")
    ...
    def save_brand(self, sender, e):
        ...
        cur = conn.cursor()
        temp = cur.execute("SELECT id FROM BrandPart WHERE title = ?",(self.txt_title.Text,)).fetchall()
        if temp:
            show_message("Ошибка", "Такой производитель уже существует", "error", "ok")
            return
        try:
            cur.execute("BEGIN TRANSACTION")
            cur.execute("INSERT INTO BrandPart (title) VALUES (?)", (self.txt_title.Text,))
            conn.commit()
            show_message("Успех", f"Производитель {self.txt_title.Text} успешно добавлена!", "info", "ok")
            self.window.Close()
